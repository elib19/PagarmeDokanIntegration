...
// Pega a cidade e estado do vendedor
$meta = get_user_meta( $package['seller_id'], 'dokan_profile_settings' );
if ( ! empty( $meta ) ) {
    if ( isset( $meta[0]['address']['city'] ) && strlen( $meta[0]['address']['city'] ) > 3 ) {
        $cidade_vendedor = $meta[0]['address']['city'];
        $estado_vendedor = $meta[0]['address']['state'];
    }
}
?>

<tr class="shipping">
    <th><?php echo wp_kses_post( $package_name ); ?> <?php echo isset( $cidade_vendedor ) ? '<br><small>(' . esc_html( $cidade_vendedor ) . ' - ' . esc_html( $estado_vendedor ) . ')</small>' : ''; ?></th>
...
<?php
// Adiciona o script para atualizar métodos de envio dinamicamente
